...
public class CPRendererLayer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
  public CPRendererLayer(RenderLayerParent<T, M> pRenderer) {
    super(pRenderer);
  }
  ...
  public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int light, T entity, float limbSwing, float limbSwingAmount, float pPartialTick, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
    ...
    if (stack.getItem() instanceof CamelPack) {
      CPRender(stack, poseStack, multiBufferSource, light, entity);
    }
  }

  public static void CPRender(ItemStack stack, PoseStack poseStack, MultiBufferSource multiBufferSource, int light, LivingEntity entity) {
    poseStack.pushPose();
    ...
    if (entity.isShiftKeyDown()) {
      poseStack.translate(0, -0.05, .5); // Lower position
      poseStack.mulPose(Axis.XP.rotationDegrees(-30)); // Forward/back swing
    }
